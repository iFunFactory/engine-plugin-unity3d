...
### v340
UDP 트랜스포트 이용 시에도 세션 연결을 보장하도록 수정했습니다. 서버 연결 시도 시 UDP 메시지가 유실될
경우에 대한 처리를 추가했으며 이는 내부적으로 이뤄지는 동작이므로 사용법은 기존과 동일합니다.
이 변경을 적용하기 위해서는 5384 이상 버전의 서버가 필요하며, 해당 서버에서 다시 빌드한 protobuf dll 을 사용해야 합니다.
