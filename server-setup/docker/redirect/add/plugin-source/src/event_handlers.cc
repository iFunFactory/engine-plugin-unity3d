...
void OnSessionClosed(const Ptr<Session> &session, SessionCloseReason reason) {
  ...
  const string account = AccountManager::FindLocalAccount(session);
...
void OnEchoMessage(const Ptr<Session> &session, const Json &message) {
  ...
  if (message_from_client == "request_redirect")
  {
    std::string account = AccountManager::FindLocalAccount(session);
    const auto user_id = to_string(session->id());

    if (account.empty()) {
      // 로그인 시킨다; 테스트니 블럭킹 콜로 때운다
      if (not AccountManager::CheckAndSetLoggedIn(user_id, session)) {
        LOG(ERROR) << "Client(account=" << account << ", session=" << session->id()
            << "): already logged-in";
        session->Close();
        return;
      }
    ...
    if (0 == Rpc::GetPeers(&peers, false)) {
      LOG(ERROR) << "No peers";
      return;
    }

    // b. 피어 선택 후 전송
    BOOST_VERIFY(AccountManager::RedirectClient(session, peers.begin()->first, ""));
  }
  else
  {
...
void RegisterEventHandlers() {
   ...
   */
  {
    ...
    /////////////////////////////////////////////
    AccountManager::RegisterRedirectionHandler([](const string &account_id, const Ptr<Session> &session, bool success, const string &extra) {
      LOG(INFO) << "Account(" << account_id << "): redirect as Session(" << session->id() << ": " << success;
    });
  }
